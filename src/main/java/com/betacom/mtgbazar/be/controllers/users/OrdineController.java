...
/**
 * Ordini del cliente — FASE C: l'utenteId nasce dal token. Spariscono
 * i @RequestParam utenteId: gli URL diventano /api/ordini,
 * /api/ordini/{id}, /api/ordini/{id}/annulla — niente piu' numero
 * utente da poter cambiare. Le transizioni restano kebab-case.
 * L'ownership check nel service (getDettaglio, annulla, ...) resta:
 * blinda "quale ordine", complementare a "chi sono" del token.
 */
@RestController
...
public class OrdineController {
    ...
    @GetMapping
    public List<OrdineDTO> list(Authentication authentication) {
        Long utenteId = SecurityUtils.utenteId(authentication);
        log.debug("GET /api/ordini utente={}", utenteId);
        return ordineS.listOrdini(utenteId);
    }
 
    @GetMapping("/{id}")
    public OrdineDTO dettaglio(@PathVariable Long id, Authentication authentication) {
        Long utenteId = SecurityUtils.utenteId(authentication);
    ...
    public List<StoricoStatoOrdineDTO> timeline(@PathVariable Long id, Authentication authentication) {
        Long utenteId = SecurityUtils.utenteId(authentication);
    ...
    public OrdineDTO annulla(@PathVariable Long id, Authentication authentication) {
        Long utenteId = SecurityUtils.utenteId(authentication);
    ...
    public OrdineDTO confermaConsegna(@PathVariable Long id, Authentication authentication) {
        Long utenteId = SecurityUtils.utenteId(authentication);
    ...
    public OrdineDTO segnalaNonConsegnato(@PathVariable Long id, Authentication authentication) {
        Long utenteId = SecurityUtils.utenteId(authentication);
    ...
    public OrdineDTO richiediReso(@PathVariable Long id, Authentication authentication) {
        Long utenteId = SecurityUtils.utenteId(authentication);
